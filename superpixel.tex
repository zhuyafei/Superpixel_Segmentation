...
目前对图像的处理大多以像素为单位，用二维矩阵来表示一幅图像，并未考虑像素之间的空间组织关系，这使得算法处理效率过低。2003年，Ren等人~\cite{fukunaga1975estimation}最早提出了超像素这一概念，所谓超像素，是指具有相似纹理、颜色、亮度等特征的相邻像素构成的图像块。它利用像素之间特征的相似程度将像素分组，可以获取图像的冗余信息，在很大程度上降低了后续图像处理任务的复杂度。

超像素生成算法大致可分为基于图论的方法、基于梯度上升的方法两类~\cite{fukunaga1975estimation} 。
...
1. 看了百度文库上的这篇文章\cite{Mean Shift Introduction}后的几点收获：
...
2）几个难懂而又关键的词语：核函数、Mean Shift向量、带宽矩阵

2. 经学习后对上述词语的理解： 

\textbf{核函数}：由网站\url{http://www.cnblogs.com/jerrylead/archive/2011/03/18/1988406.html}了解到：在机器学习领域，核函数是低维空间到高维空间的一个映射，目的是解决以下两种问题：

1）分类问题中解决线性不可分的情况

参考网址\url{http://www.360doc.com/content/14/0728/15/14106735_397653989.shtml}
，详见图~\ref{fig: kernel}
...
3. 百度文库上一篇文章\cite{mean shift understanding}后的体会：

1）知道了Mean Shift最初是用来干什么的，非参数概率密度估计是估计样本数据集的概率密度分布，而非参数概率密度梯度估计则是估计样本集在哪个点在最密集，后者即是Mean Shift的作用。

2）

4. 看了1975年的论文“The estimation of the gradient of a density function, with applications in pattern recognition"后明白了这篇论文主要是估计样本数据的多元概率密度梯度，基本思想是：某点的概率密度梯度可以用其周围一个很小的区域内的样本观测值来估计。对样本中的每一点，需要估计出其周围$k$个点的梯度向量，$k$是固定的，这些梯度向量就叫做“k最近邻mean shift簇"，对这些mean shift向量求加权平均则得到该点的密度梯度。
...
其中，
\begin{align}
v_h(X) \equiv {Y:(Y-X)^T(Y-X) \le h^2}
\end{align}

mean shift向量就是从密度函数梯度的非参数估计中推导获得的，取（1）式的最后一项就是就是mean shift向量

\begin{align}
M_h(X) \equiv \frac{1}{k} \sum_{X_i \in S_h(X)} (X_i-X)
\end{align}

举个例子，见\cite{mean shift example}
...
这里有两个问题：一是怎么确定$X$周围$k$个数据点所在的区域？二是这篇文章是估计概率密度梯度的，那么在任意点$X$处其mean shift向量与梯度方向相同，一定是指向梯度上升方向吗？怎么解释？
